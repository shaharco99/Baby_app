...
**Every `supabase/migrations/` file must be idempotent.** `create table if not exists`, `create index if not exists`, `drop policy if exists` before each `create policy`, same for triggers. Nothing here applies migrations automatically — they go on by hand, sometimes after the object was already created by hand during the work, and a plain `create` then fails forever against the live project and leaves file and database permanently out of step. `0011_device_push_tokens.sql` is the worked example. Applying one is a manual step before tagging a release; tagging first breaks sync silently, with no error on either phone.
